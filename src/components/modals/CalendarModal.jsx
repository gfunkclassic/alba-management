                ...
                <div className="p-4 bg-[#5d6c4a] text-[#f5f3e8] flex justify-between items-center border-b-2 border-[#3d472f]">
                    <div>
                        <h3 className="text-lg font-bold flex items-center gap-2">
                            ...
                            {isLocked && <span className="text-xs bg-[#8d5a4d] px-2 py-0.5 ml-2 border border-[#7a3d37]">🔒 확정 잠금</span>}
                        </h3>
                    ...
                    <div className="flex gap-2">
                        ...
                        {!isLocked && <button onClick={() => modalFileInputRef.current.click()} className="bg-[#f5f3e8] hover:bg-[#f5f3e8] px-3 py-1.5 flex items-center gap-1 text-xs border-2 border-[#3d472f] text-[#3d472f] font-bold"><Upload size={14} /> 업로드</button>}
                        <button onClick={handleDownloadPersonal} className="bg-[#f5f3e8] hover:bg-[#f5f3e8] px-3 py-1.5 flex items-center gap-1 text-xs border-2 border-[#3d472f] text-[#3d472f] font-bold"><Download size={14} /> 다운로드</button>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 grid grid-cols-1 md:grid-cols-3 gap-6 bg-[#e8e4d4]">
                    ...
                    <div className="md:col-span-2 bg-[#f5f3e8] p-4 border border-[#d4cfbf]">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                                <div className="flex bg-[#e8e4d4] p-1 border border-[#d4cfbf]">
                        ...
                        <div className="grid grid-cols-7 gap-px bg-[#c5c0b0] border border-[#d4cfbf]">
                            ...
                            {daysInMonth.map(day => {
                                ...
                                if (hasData?.reason) {
                                    ...
                                    const color = isAbsence ? 'bg-[#8d5a4d] text-white' : isLeave ? 'bg-[#5a6878] text-white' : 'bg-[#c65911] text-white';
                                ...
                                if (hasData?.overtimeReason) {
                                    badges.push({ label: `연장:${hasData.overtimeReason.length > 3 ? hasData.overtimeReason.substring(0, 3) : hasData.overtimeReason}`, color: 'bg-[#5a6878] text-white' });
                                }
                                return (
                                            ...
                                            {(displayCheckIn && displayCheckOut) && (
                                                calendarViewMode === 'TIME' ? (
                                                    <div className="text-[10px] font-medium text-[#5a5545] space-y-0.5 leading-tight">
                                                        ...
                                                        {totalDailyHours > 0 && (<div className="text-right text-[#5d6c4a] font-bold border-t border-[#ebe8db] pt-0.5 mt-0.5">{regularHours}h {displayOvertime > 0 && <span className="text-[#8d5a4d]">+{displayOvertime}</span>}</div>)}
                                                    </div>
                                                ) : (
                                                    <div className="w-full text-right">
                                                        <div className="text-sm font-black text-[#5d6c4a] -tracking-wide">₩{dailyWageAmount.toLocaleString()}</div>
                                                        {displayOvertime > 0 && <div className="text-[8px] text-[#8d5a4d] font-bold">특근포함</div>}
                                                    </div>
                                                )
                                            )}
                                        </div>
                    ...
                    {/* Right Panel */}
                    <div className="bg-[#f5f3e8] p-5 border border-[#d4cfbf] flex flex-col h-full">
                        {/* Panel Tabs */}
                        {!isLocked && (
                            <div className="flex mb-4 border border-[#d4cfbf]">
                                <button onClick={() => setRightPanel('EDIT')} className={`flex-1 py-2 text-xs font-bold flex items-center justify-center gap-1 ${rightPanel === 'EDIT' ? 'bg-[#5d6c4a] text-[#f5f3e8]' : 'text-[#7a7565] hover:bg-[#f5f3e8]'}`}>
                                    <Edit size={12} /> 개별 수정
                                </button>
                                <button onClick={() => { setRightPanel('BATCH'); setBatchResult(null); }} className={`flex-1 py-2 text-xs font-bold flex items-center justify-center gap-1 border-l-2 border-[#c5c0b0] ${rightPanel === 'BATCH' ? 'bg-[#5d6c4a] text-[#f5f3e8]' : 'text-[#7a7565] hover:bg-[#f5f3e8]'}`}>
                                    <Layers size={12} /> 일괄 적용
                                </button>
                            </div>
                        )}
                            ...
                            <>
                                ...
                                {editMode && !isLocked ? (
                                    <div className="space-y-4 flex-1 flex flex-col">
                                        <div className="bg-[#e8e4d4] p-3 border border-[#d4cfbf]">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mb-2">
                                                <div><label className="text-[10px] font-bold text-[#7a7565]">출근</label><input type="time" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-sm focus:border-[#5d6c4a] outline-none" value={form.checkIn} onChange={e => setForm({ ...form, checkIn: e.target.value })} /></div>
                                                <div><label className="text-[10px] font-bold text-[#7a7565]">퇴근</label><input type="time" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-sm focus:border-[#5d6c4a] outline-none" value={form.checkOut} onChange={e => setForm({ ...form, checkOut: e.target.value })} /></div>
                                            </div>
                                            {isEarlyLeave && <p className="text-[10px] text-[#8d5a4d] font-medium flex items-center gap-1 mb-2"><AlertTriangle size={10} /> 조기 퇴근</p>}
                                            <div className="mb-2">
                                                <label className="text-[10px] font-bold text-[#7a7565]">연장근무 (시간)</label>
                                                <div className="flex items-center gap-2">
                                                    <input type="number" step="0.5" className="flex-1 p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-sm focus:border-[#5d6c4a] outline-none" value={isNaN(form.overtime) ? '' : form.overtime} onChange={e => setForm({ ...form, overtime: e.target.value === '' ? 0 : parseFloat(e.target.value) })} />
                                                    <span className="text-[10px] text-[#7a7565]">x1.5</span>
                                                </div>
                                            </div>
                                            <div className="space-y-2">
                                                <div><label className="text-[10px] font-bold text-[#7a7565]">근태 사유</label><input type="text" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-sm focus:border-[#5d6c4a] outline-none" placeholder="결근 / 연차 / 공휴일 등" value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} /></div>
                                                ...
                                                {form.overtime > 0 && <div><label className="text-[10px] font-bold text-[#5a6878]">연장근무 사유</label><input type="text" className="w-full p-1.5 border-2 border-[#5a6878]/30 bg-[#f5f3e8] text-sm focus:border-[#5a6878] outline-none" placeholder="긴급 출하, 재고 정리 등" value={form.overtimeReason} onChange={e => setForm({ ...form, overtimeReason: e.target.value })} /></div>}
                                            </div>
                                        </div>
                                        <div className="mt-3">
                                        ...
                                        <div className="mt-auto pt-4 space-y-2 border-t-2 border-[#e8e4d4]">
                                            ...
                                            <div className="flex justify-between text-base font-black pt-2 border-t border-[#d4cfbf]"><span>합계</span><span className="text-[#5d6c4a]">₩{totalDailyPay.toLocaleString()}</span></div>
                            ...
                            <div className="space-y-3 flex-1 flex flex-col">
                                ...
                                <div className="space-y-2">
                                    <div className="grid grid-cols-2 gap-2">
                                        <div><label className="text-[10px] font-bold text-[#7a7565] block mb-1">시작일</label><input type="date" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-xs focus:border-[#5d6c4a] outline-none" value={batchForm.startDate} onChange={e => setBatchForm(f => ({ ...f, startDate: e.target.value }))} /></div>
                                        <div><label className="text-[10px] font-bold text-[#7a7565] block mb-1">종료일</label><input type="date" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-xs focus:border-[#5d6c4a] outline-none" value={batchForm.endDate} onChange={e => setBatchForm(f => ({ ...f, endDate: e.target.value }))} /></div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                                        <div><label className="text-[10px] font-bold text-[#7a7565] block mb-1">출근 시간</label><input type="time" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-xs focus:border-[#5d6c4a] outline-none" value={batchForm.checkIn} onChange={e => setBatchForm(f => ({ ...f, checkIn: e.target.value }))} /></div>
                                        <div><label className="text-[10px] font-bold text-[#7a7565] block mb-1">퇴근 시간</label><input type="time" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-xs focus:border-[#5d6c4a] outline-none" value={batchForm.checkOut} onChange={e => setBatchForm(f => ({ ...f, checkOut: e.target.value }))} /></div>
                                    </div>
                                    <div><label className="text-[10px] font-bold text-[#7a7565] block mb-1">연장 (시간)</label><input type="number" step="0.5" min="0" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-xs focus:border-[#5d6c4a] outline-none" value={batchForm.overtime} onChange={e => setBatchForm(f => ({ ...f, overtime: parseFloat(e.target.value) || 0 }))} /></div>
                                    <div><label className="text-[10px] font-bold text-[#7a7565] block mb-1">근태 사유 (선택)</label><input type="text" className="w-full p-1.5 border border-[#d4cfbf] bg-[#f5f3e8] text-xs focus:border-[#5d6c4a] outline-none" placeholder="공휴일, 연차 등" value={batchForm.reason} onChange={e => setBatchForm(f => ({ ...f, reason: e.target.value }))} /></div>
