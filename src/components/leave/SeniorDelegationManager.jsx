...
export default function SeniorDelegationManager() {
    ...
    const statusBadge = (d) => {
        ...
        if (d.start_date > today) return { label: '예정', cls: 'bg-[#a78049] text-white' };
                            ...
                            <div key={d.id} className="flex items-center gap-4 p-4 hover:bg-[#f5f3e8]">
                                ...
                                {d.is_active && d.end_date >= today && (
                                    <button onClick={() => handleRevoke(d.to_user_id, d._toName)}
                                        className="text-[10px] font-bold px-2 py-1 border-2 border-[#8d5a4d] text-[#8d5a4d] hover:bg-[#f5ebe7] flex items-center gap-1">
