...
export default function FinalApprovalInbox() {
    ...
    const approveLabel = isFinalMode ? '승인' : '팀장 대신 승인';
    ...
    return (
        <div className="bg-[#f5f3e8] border border-[#d4cfbf]">
            {rejectTarget && (
                <RejectModal
                    title={isFinalMode ? '최종 반려 사유 입력' : '팀장 대리 반려 사유 입력'}
            ...
            <ConfirmModal
                ...
                title={isFinalMode ? '승인' : '팀장 대신 승인'}
                message={
                    confirmApproveTarget
                        ? (isFinalMode
                            ? `${confirmApproveTarget._userName}님의 ${confirmApproveTarget.date} 신청을 승인하시겠습니까?`
                            : `${confirmApproveTarget._userName}님의 ${confirmApproveTarget.date} 신청을 팀장 대신 승인 처리하시겠습니까?`)
                        : ''
                }
                confirmText="승인하기"
                cancelText="취소"
            ...
            {/* PROXY 모드 안내 */}
            {mode === 'PROXY' && (
                <div className="px-4 py-2 bg-[#f5f1e3] border-b border-[#c9a66a] text-[10px] text-[#7a5a1a] leading-relaxed">
                    팀장이 직접 승인하기 어려운 경우, 실장 권한으로 팀장 승인 단계를 대신 처리합니다.<br />
                    대리승인 내역은 승인 이력에 별도로 기록됩니다.
                </div>
            )}
