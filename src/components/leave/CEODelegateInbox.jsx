...
/**
 * CEODelegateInbox — 대표 위임 수임자가 사용하는 승인함
 * delegation: { from_user_id, _fromName, start_date, end_date }
 */
export default function CEODelegateInbox({ delegation, refreshKey = 0 }) {
    const { getCEODelegateeRequests, ceoApproveLeaveRequest, ceoRejectLeaveRequest } = useAuth();
    ...
    const load = useCallback(async () => {
        ...
        setErrors({});
        try {
            const data = await getCEODelegateeRequests();
            setRequests(data);
        } catch (e) { console.error(e); }
        finally { setLoading(false); }
    }, [getCEODelegateeRequests]);
    ...
    const executeApprove = async () => {
        ...
        try {
            await ceoApproveLeaveRequest(req.id, delegation.from_user_id);
            await load();
        } catch (err) {
    ...
    const confirmReject = async (note) => {
        ...
        setProcessing(req.id);
        try {
            await ceoRejectLeaveRequest(req.id, null, null, null, note, delegation.from_user_id);
            await load();
        ...
        <div className="bg-[#f5f3e8] border-2 border-[#c5c0b0]">
            ...
            <ConfirmModal
                ...
                message={confirmApproveTarget
                    ? `${confirmApproveTarget._userName}님의 ${confirmApproveTarget.date} 신청을 ${delegation._fromName} 대표 위임으로 최종 승인하시겠습니까?`
                    : ''}
                confirmText="승인하기"
                cancelText="취소"
            />
            ...
            <div className="p-4 border-b-2 border-[#c5c0b0] flex items-center justify-between">
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] text-[#7a7565]">— {delegation._fromName} 대표 위임 중</span>
                    {requests.length > 0 && (
